...
def after_install():
    create_minimal_erpnext_doctypes()
...
def before_uninstall():
    delete_minimal_erpnext_doctypes()
...
def create_minimal_erpnext_doctypes():
    if 'erpnext' in frappe.get_installed_apps():
        frappe.msgprint("ERPNext is installed, skipping minimal Doctype creation.")
        return

    json_root = frappe.get_app_path("hrms", "stubs", "doctype")

    if not os.path.exists(json_root):
        frappe.msgprint(f"No stubs found at {json_root}")
        return   

    for doctype_folder in os.listdir(json_root):
        folder_path = os.path.join(json_root, doctype_folder)
        if os.path.isdir(folder_path):
            ...
            if os.path.exists(json_file):
                ...
                if not frappe.db.exists("DocType", doctype_json["name"]):
                    doc = frappe.get_doc({
                        "doctype": "DocType",
                        "module": doctype_json["module"],
                        "name": doctype_json["name"],
                        "document_type": doctype_json.get("document_type", "Master"),
                        "autoname": doctype_json.get("autoname", "Prompt"),
                        "fields": doctype_json.get("fields", []),
                        "permissions": doctype_json.get("permissions", []),
                        "icon": doctype_json.get("icon", "octicon octicon-file-directory"),
                        "sort_field": doctype_json.get("sort_field", "modified"),
                        "sort_order": doctype_json.get("sort_order", "DESC")
                    })
                    doc.insert(ignore_if_duplicate=True)
                    frappe.db.commit()
                    frappe.msgprint(f"Minimal Doctype '{doctype_json['name']}' created successfully.")


def delete_minimal_erpnext_doctypes():
    """
    Deletes all minimal Doctypes defined in the minimal_doctypes folder structure.
    Each Doctype is inside its own subfolder with a JSON file named <doctype_folder>.json
    """
    app_path = frappe.get_app_path("hrms")
    json_root = os.path.join(app_path, "stubs", "doctype")

    if not os.path.exists(json_root):
        frappe.msgprint(f"Folder not found: {json_root}")
        return

    # Loop through each Doctype subfolder
    for doctype_folder in os.listdir(json_root):
        folder_path = os.path.join(json_root, doctype_folder)
        if os.path.isdir(folder_path):
            json_file = os.path.join(folder_path, f"{doctype_folder}.json")
            if os.path.exists(json_file):
                ...
                doctype_name = doctype_json["name"]

                if frappe.db.exists("DocType", doctype_name):
                    doc = frappe.get_doc("DocType", doctype_name)
                    doc.delete()
                    frappe.db.commit()
                    frappe.msgprint(f"Minimal Doctype '{doctype_name}' deleted successfully.")
                else:
                    frappe.msgprint(f"Doctype '{doctype_name}' does not exist, skipping.")
